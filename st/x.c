...
xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len, int x, int y)
{
	...
	for (i = 0, xp = winx, yp = winy + font->ascent; i < len; ++i) {
		...
		/* Center the fallback glyph within the cell. */
		{
			XGlyphInfo gi;
			XftTextExtents32(xw.dpy, frc[f].font, &rune, 1, &gi);
			int glyphwidth = gi.xOff;
			if (glyphwidth > 0 && glyphwidth != (int)runewidth)
				specs[numspecs].x = (short)(xp + (runewidth - glyphwidth) / 2);
			else
				specs[numspecs].x = (short)xp;
		}
		specs[numspecs].y = (short)yp;
